...
class HeteroSecureBoostingTreeGuest(BoostingTree):
    ...
    def predict(self, data_instances:DTable):
        ...
        data_instances = self.data_alignment(data_instances) # 读数据
        self.predict_f_value(data_instances)    # 这里已经遍历完了整颗树
        if self.task_type == consts.CLASSIFICATION:
            loss_method = self.loss
    ...
    def decode_node(self, node):
        ...
        node_dict["fid"] = node.fid
        node_dict["bid"] = node.bid
        node_dict["is_leaf"] = node.is_leaf
        node_dict["left_nodeid"] = node.left_nodeid
        node_dict["right_nodeid"] = node.right_nodeid
        node_dict["sitename"] = node.sitename
        node_dict["weight"] = node.weight
        return node_dict

    def sync_model_from_host(self):
        # 用dict存储所有模型信息
        model = {}

        # 第一步, 收取host方的split_maskdicts
        # 下标
        split_maskdicts_host = self.transfer_inst.recv_data_from_hosts(-1)
        model["split_maskdicts_host"] = split_maskdicts_host[0]

        # 第二步, 取出host方的fid_map
        fid_map_host = self.transfer_inst.recv_data_from_hosts(-1)
        model["fid_map_host"] = fid_map_host[0]

        # 第三步, 取出guest方的split_maskdicts
        split_maskdicts_guest = {}
        for i in range(len(self.trees_)):
            maskdict = {}
            for k in self.trees_[i].split_maskdict:
                    maskdict[k] = self.trees_[i].split_maskdict[k]
            split_maskdicts_guest[i] = maskdict
        model["split_maskdicts_guest"] = split_maskdicts_guest
        
        # 第四步, 取出guest方的fid_map
        fid_map_guest = self.feature_name_fid_mapping
        model["fid_map_guest"] = fid_map_guest


        # 第五步, 取出guest方的树结构   
        # trees中存储多棵树，每棵树是一个list, 存储所有节点
        trees = []
        for tree in self.trees_:
            ...
            while queue:
                ...
                queue.pop(0)
                # 访问一个node
                node_dict = self.decode_node(node) # 
                tree_queue.append(node_dict)
                if node.left_nodeid != -1:
                    queue.append(tree.tree_[node.left_nodeid])
